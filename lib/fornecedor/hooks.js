...
export function useFornecedorPages({ creatorId, limit = 10 } = {}) {
  const { data, error, size, ...props } = useSWRInfinite(
    (index, previousPageData) => {
      ...
      return `/api/fornecedores?${searchParams.toString()}`;
    },
    fetcher,
    {
  ...
  const isReachingEnd =
    isEmpty || (data && data[data.length - 1]?.fornecedores?.length < limit);
...
export function useFornecedorSelect({ creatorId, limit = 10 } = {}) {
  const { data, error, size, ...props } = useSWRInfinite(
    (index, previousPageData) => {
      // reached the end
      if (previousPageData && previousPageData.posts.length === 0) return null;
