...
export function useItemPages({ creatorId, limit = 10 } = {}) {
  const { data, error, size, ...props } = useSWRInfinite(
    (index, previousPageData) => {
      ...
      return `/api/itens?${searchParams.toString()}`;
    },
    fetcher,
    {
  ...
  const isReachingEnd =
    isEmpty || (data && data[data.length - 1]?.item?.length < limit);
...
export function useItemSelect({ creatorId, limit = 100 } = {}) {
  const { data, error, size, ...props } = useSWRInfinite(
    (index, previousPageData) => {
      if (previousPageData && previousPageData.posts.length === 0) return null;
      const searchParams = new URLSearchParams();
