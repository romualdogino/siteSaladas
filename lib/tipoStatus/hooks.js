...
export function useTipoStatusPages({ creatorId, limit = 10 } = {}) {
  const { data, error, size, ...props } = useSWRInfinite(
    (index, previousPageData) => {
      ...
      return `/api/tipoStatus?${searchParams.toString()}`;
    },
    fetcher,
    {
  ...
  const isReachingEnd =
    isEmpty || (data && data[data.length - 1]?.tipoStatus?.length < limit);
