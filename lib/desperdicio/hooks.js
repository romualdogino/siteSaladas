...
export function useDesperdicioPages({ creatorId, limit = 10 } = {}) {
  const { data, error, size, ...props } = useSWRInfinite(
    (index, previousPageData) => {
      ...
      return `/api/desperdicios?${searchParams.toString()}`;
    },
    fetcher,
    {
  ...
  const isReachingEnd =
    isEmpty || (data && data[data.length - 1]?.desperdicios?.length < limit);
