...
export function usePedidosItensAvulsosPages({ creatorId, limit = 100 } = {}) {
  const { data, error, size, ...props } = useSWRInfinite(
    (index, previousPageData) => {
      ...
      return `/api/pedidosItensAvulsos?${searchParams.toString()}`;
    },
    fetcher,
    {
  ...
  const isReachingEnd =
    isEmpty || (data && data[data.length - 1]?.grupos?.length < limit);
